...
print("Github download : https://github.com/lanexadev/lnx_piggyback")
...
RegisterCommand('porter', function(source, args)
	...
	if not piggyBackInProgress then
		...
		if closestDistance ~= -1 and closestDistance <= 3 then
			...
			local length = 100000
			local spin = 0.0
			local controlFlagsMe = 49
			local controlFlagTarget = 33
			local animFlagTarget = 1

			_TriggerServerEvent('cmg2_animations:sync', lib, anim1, anim2, distans, distans2, height, target, length, spin, controlFlagsMe, controlFlagTarget, animFlagTarget)
		end
	else
...
Citizen.CreateThread(function()
	while true do
		if holdingHostage then
			...
			ESX.Game.Utils.DrawText3D(plyCoords, 'Appuyer sur [G] pour relâcher, [Z] pour tuer', 0.5)
